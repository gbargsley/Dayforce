...
function Get-AgMovementTimeline {
    [CmdletBinding()]
    param(
        [object[]]$Incidents
    )
    ...
    $ordered = @($Incidents | Sort-Object StartTime, IncidentId)
    $rows = foreach ($incident in $ordered) {
        $beforeNode = if ($incident.PrimaryBefore) { $incident.PrimaryBefore } elseif ($incident.FailureNode) { $incident.FailureNode } else { 'Unknown' }
        $afterNode = if ($incident.PrimaryAfter) { $incident.PrimaryAfter } elseif ($incident.RecoveryNode) { $incident.RecoveryNode } else { 'Unknown' }
        $failureNode = if ($incident.FailureNode) { $incident.FailureNode } elseif ($beforeNode) { $beforeNode } else { 'Unknown' }
        $recoveryNode = if ($incident.RecoveryNode) { $incident.RecoveryNode } elseif ($afterNode) { $afterNode } else { 'Unknown' }

        $triggerText = switch -Regex ([string]$incident.RootCause) {
            'XTP|In-Memory OLTP|memory-optimized|memory optimized|memory pressure|memory error|FAIL_PAGE_ALLOCATION|MEMORYCLERK_XTP' { "XTP memory pressure on $failureNode" }
            'restart|shutdown' { "SQL Server restart/shutdown on $failureNode" }
            'lease timeout' { "Lease timeout impacting $failureNode" }
            'health check' { "Health check failure impacting $failureNode" }
            'connection|endpoint' { "Connectivity issue affecting $failureNode" }
            default { if ($incident.RootCause) { [string]$incident.RootCause } else { 'AG role transition' } }
        }

        $transitionText = if ($failureNode -and $recoveryNode -and $failureNode -ne $recoveryNode) {
            "Failover: $failureNode -> $recoveryNode"
        }
        elseif ($failureNode -and $recoveryNode) {
            "Recovery on $recoveryNode while $failureNode rejoined"
        }
        elseif ($recoveryNode) {
            "Stabilized on $recoveryNode"
        }
        else {
            'Stabilization not clearly established'
        }

        $afterText = if ($incident.PrimaryChanged -and $afterNode -and $beforeNode -and $beforeNode -ne $afterNode) {
            "After stabilization: $beforeNode -> $afterNode"
        }
        elseif ($afterNode -and $failureNode -and $afterNode -ne $failureNode) {
            "After stabilization: $afterNode remained primary; $failureNode rejoined"
        }
        elseif ($afterNode) {
            "After stabilization: $afterNode remained primary"
        }
        else {
            'After stabilization: not clearly established'
        }

        $startText = if ($incident.FailoverStartTime) { $incident.FailoverStartTime.ToString('yyyy-MM-dd HH:mm:ss') } else { $incident.StartTime.ToString('yyyy-MM-dd HH:mm:ss') }
        $completeText = if ($incident.FailoverCompleteTime) { $incident.FailoverCompleteTime.ToString('yyyy-MM-dd HH:mm:ss') } else { $incident.EndTime.ToString('yyyy-MM-dd HH:mm:ss') }
        $summaryLine = "$beforeNode -> $triggerText -> $transitionText -> $afterText"
        ...
        [pscustomobject]@{
            ...
            FailoverStartTime = $incident.FailoverStartTime
            FailoverCompleteTime = $incident.FailoverCompleteTime
            SummaryLine       = $summaryLine
            PrimaryChanged    = [bool]$incident.PrimaryChanged
        }
    }
...
function Convert-ToAgNarrative {
    ...
    if ($MovementTimeline -and $MovementTimeline.Count -gt 0) {
        $lines.Add('Movement timeline:') | Out-Null
        foreach ($step in $MovementTimeline) {
            $lines.Add(("  Incident {0} [{1}] {2} -> {3}" -f $step.IncidentId, $step.Severity, $step.BeforeNode, $step.AfterText)) | Out-Null
            $lines.Add(("    {0}" -f $step.TriggerText)) | Out-Null
            $lines.Add(("    {0}" -f $step.TransitionText)) | Out-Null
        }
        $lines.Add('') | Out-Null
    }
    $lines.Add('') | Out-Null
...
function Export-AgFailureHtml {
    ...
    $movementTimeline = @(
        Get-SafePropertyValue -Object $Report -Name 'MovementTimeline' -Default @()
    )
...
"@
    ...
    $movementCards = foreach ($step in $movementTimeline) {
        $cls = if ([string]$step.Severity -eq 'High') { 'incident-high' } elseif ([string]$step.Severity -eq 'Medium') { 'incident-medium' } else { 'incident-low' }
        $incidentId = [System.Net.WebUtility]::HtmlEncode([string]$step.IncidentId)
        $startText = [System.Net.WebUtility]::HtmlEncode([string]$step.StartText)
        $completeText = [System.Net.WebUtility]::HtmlEncode([string]$step.CompleteText)
        $beforeNode = [System.Net.WebUtility]::HtmlEncode([string]$step.BeforeNode)
        $triggerText = [System.Net.WebUtility]::HtmlEncode([string]$step.TriggerText)
        $transitionText = [System.Net.WebUtility]::HtmlEncode([string]$step.TransitionText)
        $afterText = [System.Net.WebUtility]::HtmlEncode([string]$step.AfterText)
        $summaryLine = [System.Net.WebUtility]::HtmlEncode([string]$step.SummaryLine)
        $rootCause = [System.Net.WebUtility]::HtmlEncode([string]$step.RootCause)
        $rootCauseConfidence = [System.Net.WebUtility]::HtmlEncode([string]$step.RootCauseConfidence)
        $confidence = [System.Net.WebUtility]::HtmlEncode([string]$step.ConfidenceLabel)
        $score = [System.Net.WebUtility]::HtmlEncode([string]$step.ConfidenceScore)
        $status = [System.Net.WebUtility]::HtmlEncode([string]$step.Status)
        $failureNode = [System.Net.WebUtility]::HtmlEncode([string]$step.FailureNode)
        $recoveryNode = [System.Net.WebUtility]::HtmlEncode([string]$step.RecoveryNode)

        @"
<div class='movement-card $cls'>
  <div class='movement-card-head'>
    <div class='movement-card-title'>Incident $incidentId · $confidence ($score/100) · $status</div>
    <div class='movement-card-subtitle'>$startText to $completeText · Root cause: $rootCause ($rootCauseConfidence)</div>
  </div>
  <div class='movement-flow'>
    <div class='movement-stage stage-before'>
      <div class='movement-stage-label'>Before</div>
      <div class='movement-stage-value'>$beforeNode</div>
    </div>
    <div class='movement-arrow'>→</div>
    <div class='movement-stage stage-trigger'>
      <div class='movement-stage-label'>Trigger</div>
      <div class='movement-stage-value'>$triggerText</div>
    </div>
    <div class='movement-arrow'>→</div>
    <div class='movement-stage stage-transition'>
      <div class='movement-stage-label'>Transition</div>
      <div class='movement-stage-value'>$transitionText</div>
    </div>
    <div class='movement-arrow'>→</div>
    <div class='movement-stage stage-after'>
      <div class='movement-stage-label'>After</div>
      <div class='movement-stage-value'>$afterText</div>
    </div>
  </div>
  <div class='movement-card-foot'>
    <div><strong>Failure node:</strong> $failureNode</div>
    <div><strong>Recovery node:</strong> $recoveryNode</div>
    <div><strong>Sequence:</strong> $summaryLine</div>
  </div>
</div>
"@
    }
    if (-not $movementCards) { $movementCards = @() }
...
<head>
    ...
    <style>
        ...
        .movement-card { margin: 14px 0 18px 0; padding: 12px 14px; border: 1px solid #d8d8d8; border-left-width: 8px; border-radius: 10px; background: #fff; }
        .movement-card-head { margin-bottom: 10px; }
        .movement-card-title { font-weight: 700; font-size: 15px; }
        .movement-card-subtitle { color: #555; font-size: 12px; margin-top: 3px; }
        .movement-flow { display: flex; gap: 10px; align-items: stretch; flex-wrap: wrap; }
        .movement-stage { flex: 1 1 160px; min-width: 160px; padding: 10px 12px; border: 1px solid #d2d2d2; border-radius: 10px; background: #f9f9f9; }
        .stage-before { background: #fff1f1; }
        .stage-trigger { background: #fff7e6; }
        .stage-transition { background: #eef6ff; }
        .stage-after { background: #edfdf0; }
        .movement-stage-label { font-size: 11px; text-transform: uppercase; letter-spacing: .04em; color: #666; margin-bottom: 4px; font-weight: 700; }
        .movement-stage-value { font-size: 13px; line-height: 1.4; }
        .movement-arrow { display: flex; align-items: center; font-size: 28px; font-weight: 700; color: #888; padding: 0 4px; }
        .movement-card-foot { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 8px; margin-top: 10px; font-size: 12px; color: #333; }
        @media (max-width: 900px) { .movement-card-foot { grid-template-columns: 1fr; } }
    </style>
</head>
<body>
    ...
    <p class="tiny">Each card shows the path from pre-event to trigger, failover, and post-stabilization. Multiple failovers appear as separate cards in time order.</p>
    $([string]::Join([Environment]::NewLine, $movementCards))
...
function Invoke-DbaAgErrorLogReview {
    ...
    if ($sortedEvents.Count -gt 0) {
        ...
        $movementTimeline = @(Get-AgMovementTimeline -Incidents $sortedIncidents)
    ...
    $result | Add-Member -NotePropertyName MovementTimeline -NotePropertyValue @($movementTimeline)

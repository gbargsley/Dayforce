...
/****** Object:  StoredProcedure [dbo].[DatabaseBackup]    Script Date: 2/20/2026 4:17:11 PM ******/
SET ANSI_NULLS ON
GO
...
--//Date: 2022-01-19
--//Changed by: Jeremy Malabar
--//Reason: Fix bug related to DF Hub Media backup discrepancy in backup timestamp
--//Change:Setting the @currentdate variable to be the same as the databasestarttime in the queuedatabase table
--//Updated line: line 2856 
--//Rollback step:Uncomment line 2857 (Original Code) and comment line 2858
--//Search Key Word: JDM
...
-------------------------------------------------------------------------------------------------------
  ...
  BEGIN
    ...
    BEGIN
	  ...
	  --Code added by JDM to fix issue related to Hub media backup backup name discrepancy in the timestamp
	  --Setting the @currentdate variable to be the same the databasestarttime in the queuedatabase table
	  --Uncomment line 2856 (Original Code) to rollback the change and comment line 2857 and 2858
      --SET @CurrentDate = SYSDATETIME()
	  SET @CurrentDate = (select DatabaseStartTime from dbo.QueueDatabase where QueueID=@QueueID and DatabaseName=@CurrentDatabaseName)
	  SET @CurrentDate=(select isnull(@CurrentDate,SYSDATETIME()))
